...
class Login extends Component {
    ...
    handleSubmit = (e) => {
      ...
      this.props.form.validateFields((err, values) => {
        if (!err) {
            const url = `${this.props.optionapp[0].serverUrl}/CheckOwner.php`; // проверяем владельца
            fetch(url, {
              method: 'POST',
              body: JSON.stringify(
              {
                ...
                if (responseJsonFromServer.owner[0].status === "1") {
                    this.props.onCheckUser(responseJsonFromServer.owner[0]);  // вызываем action
                    Cookies.set('cookiename', responseJsonFromServer.owner[0].chUIDStaff, { expires: 365 , path: '/' }); // устанавливаем куки
                } else {
    ...
    // устанавливаем статус пользователя (админ или сотрудник)
    onStaffType = (e) => {
      this.setState({
